...
Throwing an exception from `execute()` or `authorize()` is how the code API surfaces errors. The engine translates each into a GraphQL error with a machine-readable `extensions.code` and a matching HTTP status. All built-in exceptions live in `Automattic\WooCommerce\Api`.
