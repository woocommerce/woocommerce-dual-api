...
| Maximum query length | `woocommerce_graphql_max_query_length` (`OPTION_MAX_QUERY_LENGTH`) | number | `65536` | Rejects queries longer than this many bytes with `400 BAD_USER_INPUT` before they're parsed, cached or validated. Hash-only APQ requests carry no query and aren't subject to it. Falls back to default when unset or non-positive. |
...
Refreshing a file that already exists is always allowed. The directory is measured without a lock, so concurrent cache misses can overshoot the limits by a few files. All three are limits on persistence, not on what the endpoint accepts: a query beyond them still runs, up to the [maximum query length](#settings) setting, which is the hard limit on what an endpoint accepts at all. They're filterable (see below); setting one to `0` removes it. The object cache has no count or size bound of its own beyond the cacheable query size, its eviction policy and the TTL.
