 ...
 * - Repeated selections of the same field or typed inline fragment => their
 *   sub-selections are unioned, whatever the order, as GraphQL merges them
	...
	private static function extract_selection_set( ?SelectionSetNode $selection_set, array $variable_values, array $fragments, array &$expanded_fragments ): array {
		...
		foreach ( $selection_set->selections as $selection ) {
			if ( $selection instanceof FieldNode ) {
				// Merge rather than assign: the same field can appear more than
				// once in a selection set (directly, or via fragments expanded
				// earlier in the loop), and GraphQL unions those selections.
				$result = self::merge_selections(
					$result,
					array( $selection->name->value => self::build_field_entry( $selection, $variable_values, $fragments, $expanded_fragments ) )
				);
				...
				if ( null === $selection->typeCondition ) {
					...
					$key            = '...' . $selection->typeCondition->name->value;
					$result[ $key ] = self::merge_selections( $result[ $key ] ?? array(), $sub );
				}
