 ...
 *
 * In the plugin distribution there is no feature flag: the plugin being
 * active is what enables the dual API, and the plugin bootstrap only
 * registers the autoloader (and calls register()) on PHP 8.1+, on
 * WooCommerce versions that don't ship the engine in core themselves.
 */
class Main {
	/**
	 ...
	 *
	 * In the plugin distribution the plugin being active is the feature
	 * switch, so only the PHP 8.1+ requirement is left to check.
	 *
	 * @return bool
	 */
	 ...
	 * endpoint" rule so every endpoint applied through
	 * {@see self::register_graphql_endpoint()} honours the same settings.
	 *
	...
	/**
	 * Register the engine's WordPress hooks: the settings section and the
	 * OPcache cleanup action handler. Called once from the plugin bootstrap
	 * after WooCommerce has loaded.
	 *
	 * The plugin ships no GraphQL endpoint of its own: WooCommerce core's
	 * endpoint belonged to core's proof-of-concept code API, which stayed
	 * behind in core. Endpoints are registered by consumer plugins through
	 * {@see self::register_graphql_endpoint()}.
	 */
	 ...
	 *
	 * Returns null when PHP is < 8.1, so callers can invoke this
	 * unconditionally from inside their own rest_api_init handler.
	 *
	 ...
	 *
	 * When PHP is < 8.1 this is a silent no-op.
	 *
