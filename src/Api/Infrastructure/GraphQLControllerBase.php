...
/**
 * Handles incoming GraphQL requests over the WordPress REST API.
 *
 * Abstract: the autogenerated `GraphQLController` subclass emitted by
 * ApiBuilder for each plugin that ships a dual API is the concrete class,
 * and {@see Main::register_graphql_endpoint()} attaches its
 * {@see self::handle_request()} to a REST route. The public surface plugins
 * extend is engine-decoupled — the abstract {@see self::build_schema()}
 * returns {@see Schema}, a stable subclass of the underlying engine's schema
 * type, so a future engine swap doesn't break already-committed autogen trees.
 */
abstract class GraphQLControllerBase {
	...
	/**
	 * Maximum number of errors included in a response.
	 *
	 * Validation rules report one error per offending node, so a document
	 * can produce as many errors as it has fields. The response keeps the
	 * first this many and says how many were left out in
	 * `extensions.omittedErrors`. The HTTP status is still derived from all
	 * of them.
	 */
	public const MAX_ERRORS_PER_RESPONSE = 100;
	...
	private function process_request( \WP_REST_Request $request, object $principal ): \WP_REST_Response {
		...
		// of the stock rules don't (field-merge checking compares pairs of
		// fields), and the validator runs every rule to completion before
		// reporting anything. Rejecting oversized documents here keeps them
		// away from the expensive rules. The complexity-rule instance is kept
		// so its computed score can be surfaced in the debug extensions.
		$complexity_rule = new QueryComplexityRule( self::get_max_query_complexity() );
		$complexity_rule->setRawVariableValues( $variables );
		$limit_errors = $this->validate_document( $schema, $source, array( new QueryDepthRule( self::get_max_query_depth() ), $complexity_rule ) );

		if ( ! empty( $limit_errors ) ) {
			$result = new ExecutionResult( null, $limit_errors );
		} else {
			// 7. Validate against the full rule set and execute. The context
			// value is an ArrayObject (not a plain array) so root resolvers can
			// mutate it — specifically to thread the root query's metadata into
			// `$context['_query_metadata']` for downstream field-level
			// authorization gates. ArrayObject preserves the `$context['key']`
			// read syntax via ArrayAccess. The context carries the resolved
			// principal through to autogenerated resolvers, which expose it as
			// the `_principal` infrastructure parameter when commands declare
			// it on their authorize()/execute() methods. Request-derived data
			// that resolvers need is carried by the principal class itself —
			// populated by the PrincipalResolver, the only component wired to
			// the HTTP transport.
			$validation_rules = DocumentValidator::allRules();
			// The stock field-merge rule can take quadratic time and memory
		...
		// 10. Cap the number of errors sent back, after the status has been
		// derived from all of them.
		$output = $this->truncate_errors( $output );
		$status = $this->pick_status( $default, $output, $request );
	...
	/**
	 * Validate a document against a subset of rules, the way
	 * {@see GraphQL::executeQuery()} does internally: rules that throw a
	 * GraphQL {@see Error} (the complexity rule does, on variables that
	 * don't match their definitions) have the error reported as the
	 * validation outcome instead of propagating.
	 *
	 * @param Schema       $schema   The schema to validate against.
	 * @param DocumentNode $document The parsed document.
	 * @param array        $rules    The validation rules to apply.
	 * @return list<Error> The validation errors, empty when the document passes.
	 */
	private function validate_document( Schema $schema, DocumentNode $document, array $rules ): array {
		try {
			return array_values( DocumentValidator::validate( $schema, $document, $rules ) );
		} catch ( Error $e ) {
			return array( $e );
		}
	}
	...
	/**
	 * Keep only the first {@see self::MAX_ERRORS_PER_RESPONSE} errors of a
	 * response body, recording how many were dropped in
	 * `extensions.omittedErrors`.
	 *
	 * @param array $output The response body.
	 * @return array The response body with at most MAX_ERRORS_PER_RESPONSE errors.
	 */
	private function truncate_errors( array $output ): array {
		$omitted = count( $output['errors'] ?? array() ) - self::MAX_ERRORS_PER_RESPONSE;
		if ( $omitted <= 0 ) {
			return $output;
		}

		$output['errors']                      = array_slice( $output['errors'], 0, self::MAX_ERRORS_PER_RESPONSE );
		$output['extensions']['omittedErrors'] = $omitted;
