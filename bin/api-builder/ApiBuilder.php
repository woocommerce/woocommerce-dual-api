 ...
 *
 * The builder is parameterised so any plugin that ships its own code-API
 * classes can generate a GraphQL layer for them. Each caller supplies the
 * input directory/namespace to scan, the output directory/namespace to
 * generate into, and the text domain the generated code translates its
 * descriptions with. {@see self::for_plugin()} derives all of these from
 * the conventional plugin layout.
 */
class ApiBuilder {
	private const TEMPLATES_DIR = __DIR__ . '/code-templates';

	/**
	 * Character class a text domain must match to be emitted verbatim into
	 * single-quoted PHP string literals in the generated code.
	 */
	private const TEXT_DOMAIN_PATTERN = '/^[A-Za-z0-9_\-]+$/';

	/**
	 * Absolute path to the directory containing the code-API classes to scan.
	 *
	 * @var string
	 */
	...
	/**
	 * Absolute path to the directory the generated code is written to; wiped on each build.
	 *
	 * @var string
	 */
	...
	/**
	 * PSR-4 namespace that maps to {@see self::$api_dir}, without leading or trailing backslashes.
	 *
	 * @var string
	 */
	...
	/**
	 * PSR-4 namespace that maps to {@see self::$autogenerated_dir}, without leading or trailing backslashes.
	 *
	 * @var string
	 */
	...
	/**
	 * Text domain the generated code passes to `__()` for schema descriptions.
	 *
	 * @var string
	 */
	...
	/**
	 * Directory `composer dump-autoload` runs in after generation; null skips autoload regeneration.
	 *
	 * @var ?string
	 */
	...
	/**
	 * Absolute path to the phpcbf executable used to format the generated files.
	 *
	 * @var string
	 */
	...
	/**
	 * Creates a builder for the given input and output layout.
	 *
	 * @param string  $api_dir                 Absolute path to the directory containing the code-API classes to scan.
	 * @param string  $autogenerated_dir       Absolute path to the directory where generated code will be written. The directory is wiped on each build.
	 * @param string  $api_namespace           PSR-4 namespace that maps to $api_dir.
	 * @param string  $autogenerated_namespace PSR-4 namespace that maps to $autogenerated_dir.
	 * @param string  $text_domain             Text domain the generated code passes to `__()` for schema descriptions; normally the consuming plugin's own.
	 ...
	 * @param ?string $phpcbf_path             Absolute path to a phpcbf executable used to format generated files. Null = the copy installed with this plugin's development dependencies. To skip linting entirely, pass $skip_linter to build().
	 *
	 * @throws \InvalidArgumentException If $text_domain contains characters other than letters, digits, underscores and hyphens.
	 */
	public function __construct(
		string $api_dir,
		string $autogenerated_dir,
		string $api_namespace,
		string $autogenerated_namespace,
		string $text_domain,
		?string $composer_working_dir = null,
		?string $phpcbf_path = null
	) {
		if ( ! preg_match( self::TEXT_DOMAIN_PATTERN, $text_domain ) ) {
			throw new \InvalidArgumentException(
				"ApiBuilder: invalid text domain \"{$text_domain}\". Use letters, digits, underscores and hyphens only."
			);
		}

		$this->api_dir                 = $api_dir;
		$this->autogenerated_dir       = $autogenerated_dir;
		$this->api_namespace           = trim( $api_namespace, '\\' );
		$this->autogenerated_namespace = trim( $autogenerated_namespace, '\\' );
	...
	/**
	 * Configure an ApiBuilder by convention for a plugin.
	 *
	 ...
	 * `$plugin_root`. The text domain defaults to the plugin directory's
	 * name, which is the conventional text domain of a WordPress plugin.
	 * phpcbf defaults to the copy installed with the WooCommerce Dual API
	 * plugin's development dependencies (callers that want a different style
	 * pass --phpcbf-path to the CLI instead of using this factory).
	 *
	 * @param string  $plugin_root      Absolute path to the plugin repository root.
	 * @param string  $namespace_prefix Top-level PSR-4 namespace the plugin publishes under (e.g. 'Automattic\\WooCommerceSimpleEvents').
	 * @param ?string $text_domain      Text domain for the generated code's descriptions. Null = the base name of $plugin_root.
	 */
	public static function for_plugin( string $plugin_root, string $namespace_prefix, ?string $text_domain = null ): self {
		...
		return new self(
			...
			$text_domain ?? basename( $plugin_root ),
			$plugin_root
		);
	}

	/**
	 * Turn-key entry point for a plugin's `bin/build-api.php` script.
	 *
	 ...
	 *
	 * Keeps the plugin-side script tiny: after locating the WooCommerce Dual
	 * API plugin and requiring its autoloader (which is what makes this class
	 * available), the plugin only needs to call this method with its own root
	 * path and namespace prefix.
	 *
	 * @param string  $plugin_root      Absolute path to the plugin repository root.
	 * @param string  $namespace_prefix Top-level PSR-4 namespace the plugin publishes under.
	 * @param ?string $text_domain      Text domain for the generated code's descriptions. Null = the base name of $plugin_root.
	 */
	public static function run_for_plugin( string $plugin_root, string $namespace_prefix, ?string $text_domain = null ): void {
		...
		try {
			self::for_plugin( $plugin_root, $namespace_prefix, $text_domain )->build( $skip_linter );
		} catch ( \Throwable $e ) {
	...
	/**
	 * Every class discovered under the API directory, keyed by FQCN, with its kind and whether it is ignored.
	 *
	 * @var array<string, array{class: \ReflectionClass<object>|\ReflectionEnum<\UnitEnum>, kind: string, ignored: bool}>
	 */
	private array $classes = array();

	/**
	 * Map of PHP FQCN => GraphQL name.
	 *
	...
	/**
	 * Discovered connections, keyed by node type FQCN: ['node_type' => FQCN, 'source' => string].
	 *
	 * @var array<string, array{node_type: string, source: string}>
	 */
	private array $connections = array();

	/**
	 * Map of interface trait FQCN => list of output type FQCNs that use it.
	 *
	 * @var array<string, string[]>
	 */
	...
	/**
	 * Optional FQCN of a user-provided `<api_namespace>\Infrastructure\ClassResolver` class with a public static `resolve_class(string): object` method; null when absent.
	 *
	 * @var ?string
	 */
	...
	/**
	 * Optional FQCN of a user-provided `<api_namespace>\Infrastructure\PrincipalResolver` class; null when absent.
	 *
	 * @var ?string
	 */
	...
	/**
	 * Optional FQCN of a user-provided `<api_namespace>\Infrastructure\HttpStatusResolver` class with a public `resolve_status(int, array, \WP_REST_Request): int` method; null when absent.
	 *
	 * @var ?string
	 */
	...
	/**
	 * Number of query resolvers generated, for the build summary.
	 *
	 * @var int
	 */
	...
	/**
	 * Number of mutation resolvers generated, for the build summary.
	 *
	 * @var int
	 */
	...
	/**
	 * Number of output types generated, for the build summary.
	 *
	 * @var int
	 */
	...
	/**
	 * Number of input types generated, for the build summary.
	 *
	 * @var int
	 */
	...
	/**
	 * Number of enum types generated, for the build summary.
	 *
	 * @var int
	 */
	...
	/**
	 * Number of scalar types generated, for the build summary.
	 *
	 * @var int
	 */
	...
	/**
	 * Number of interface types generated, for the build summary.
	 *
	 * @var int
	 */
	...
	/**
	 * Run the whole build: detect the infrastructure classes, discover and validate the API classes, then regenerate the GraphQL layer.
	 *
	 * Exits the process with status 1 when validation records any error.
	 *
	 * @param bool $skip_linter True to skip running phpcbf on the generated files.
	 */
	 ...
	 * build-time error.
	 *
	 * @param string            $fqcn   FQCN of the attribute class, for error messages.
	 ...
	 *
	 * @param string               $fqcn            FQCN of the attribute class.
	 * @param \ReflectionParameter $principal_param The offending principal parameter.
	 */
	 ...
	 * @param string               $expected The type name the parameter must declare.
	 */
	 ...
	 * about formatting.
	 *
	 * @param mixed $value Value to render as PHP source.
	 */
	private static function render_php_literal( mixed $value ): string {
		return var_export( $value, true ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_var_export -- Design-time code generator emitting a PHP literal, not debug output.
	}
	...
	/**
	 * Scan the API directory, reflect every PHP class found and record its kind, ignored status and GraphQL name.
	 */
	private function discover(): void {
		...
		foreach ( $iterator as $file ) {
			...
			if ( null === $kind || 'attribute' === $kind || 'exception' === $kind || 'infrastructure' === $kind ) {
				continue;
			}
	...
	/**
	 * Map a PHP file under the API directory to the FQCN its PSR-4 layout implies.
	 *
	 * @param string $filepath Absolute path of the PHP file.
	 */
	...
	/**
	 * Derive a class's kind (query, mutation, type, ...) from the first namespace segment below the API namespace.
	 *
	...
	/**
	 * Dispatch each discovered, non-ignored class to the validator for its kind.
	 */
	...
	/**
	 * Validate a query/mutation class: `execute()` presence, authorization, and `#[ReturnType]` consistency.
	 *
	 * @param string           $fqcn FQCN of the command class.
	 * @param \ReflectionClass $ref  Reflection of the command class.
	 *
	...
	/**
	 * Validate the public properties of an output type class.
	 *
	 * @param string           $fqcn FQCN of the output type class.
	 * @param \ReflectionClass $ref  Reflection of the output type class.
	 *
	 * @phpstan-param \ReflectionClass<object> $ref
	 */
	...
	/**
	 * Validate the public properties of an input type class.
	 *
	 * @param string           $fqcn FQCN of the input type class.
	 * @param \ReflectionClass $ref  Reflection of the input type class.
	 *
	 * @phpstan-param \ReflectionClass<object> $ref
	 */
	...
	/**
	 * Check that a public property declares a type, and that an `array` property carries `#[ArrayOf]` or `#[ConnectionOf]`.
	 *
	 * @param \ReflectionProperty $prop       The property to check.
	 * @param string              $context    Either 'output' or 'input'.
	 * @param string              $class_name Short name of the declaring class, for error messages.
	 */
	...
	/**
	 * Validate that an enum is backed (string or int).
	 *
	 * @param string          $fqcn FQCN of the enum.
	 * @param \ReflectionEnum $ref  Reflection of the enum.
	 *
	 * @phpstan-param \ReflectionEnum<\UnitEnum> $ref
	 */
	...
	/**
	 * Validate that a scalar class declares `serialize` and `parse` methods.
	 *
	 * @param string           $fqcn FQCN of the scalar class.
	 * @param \ReflectionClass $ref  Reflection of the scalar class.
	 *
	 * @phpstan-param \ReflectionClass<object> $ref
	 */
	...
	/**
	 * Validate that an interface is a trait and that its public properties are well-typed.
	 *
	 * @param string           $fqcn FQCN of the interface trait.
	 * @param \ReflectionClass $ref  Reflection of the interface trait.
	 *
	 * @phpstan-param \ReflectionClass<object> $ref
	 */
	 ...
	 *     error: ?string,
	 *     descriptors: list<array{attribute: string, args: list<mixed>}>,
	 * }
	 */
	 ...
	 *
	 * @phpstan-param \ReflectionClass<object>|\ReflectionProperty $source
	 *
	 ...
	 *
	 * @phpstan-param \ReflectionClass<object>|\ReflectionProperty $reflector
	 *
	 * @return list<array{attribute: string, args: list<mixed>}>
	 */
	 ...
	 *
	 * @param \ReflectionProperty $prop                   The property whose gate is being resolved.
	 * @param array               $type_level_usages      Class-level authorization usages of the declaring type, ANDed into the field gate.
	 * @param array               $type_level_descriptors Class-level authorization descriptors of the declaring type, merged into the field's descriptors.
	 *
	 * @return array{usages: list<array{fqcn: string, args_php: string, is_public_access: bool}>, attribute_expr: string, first_attribute_short: ?string, descriptors: list<array{attribute: string, args: list<mixed>}>}
	 */
	...
	private function render_attribute_args_php( string $attr_fqcn, array $args ): string {
		...
		foreach ( $args as $key => $value ) {
			$exported = var_export( $value, true ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_var_export -- Design-time code generator emitting a PHP literal, not debug output.
			if ( is_string( $key ) ) {
				$pieces[] = "{$key}: {$exported}";
			} else {
				$pieces[] = $exported;
	...
	/**
	 * Delete the autogenerated directory and everything in it, if it exists.
	 */
	...
	/**
	 * Create the autogenerated directory tree the generators write into.
	 */
	...
	/**
	 * Generate every output file: types, resolvers, connections, root types, type registry and controller.
	 */
	private function generate(): void {
		...
		foreach ( $this->classes as $fqcn => $info ) {
			if ( ! $info['ignored'] && 'interface' === $info['kind'] ) {
		...
		foreach ( $this->classes as $fqcn => $info ) {
			if ( $info['ignored'] || 'type' !== $info['kind'] ) {
				continue;
			}
	...
	/**
	 * Register a connection for every query/mutation whose `execute()` carries `#[ConnectionOf]`.
	 *
	 * @param array $queries   Map of FQCN => reflection of the query classes.
	 * @param array $mutations Map of FQCN => reflection of the mutation classes.
	 */
	...
	/**
	 * Generate the GraphQL enum type class for a PHP backed enum.
	 *
	 * @param string          $fqcn FQCN of the enum.
	 * @param \ReflectionEnum $ref  Reflection of the enum.
	 *
	...
	/**
	 * Generate the GraphQL scalar type class for a scalar class.
	 *
	 * @param string           $fqcn FQCN of the scalar class.
	 * @param \ReflectionClass $ref  Reflection of the scalar class.
	 *
	...
	/**
	 * Generate the GraphQL interface type class for an interface trait, including its resolveType map.
	 *
	 * @param string           $fqcn FQCN of the interface trait.
	 * @param \ReflectionClass $ref  Reflection of the interface trait.
	 *
	...
	/**
	 * Generate the GraphQL object type class for an output type, wiring in the interfaces its traits map to.
	 *
	 * @param string           $fqcn FQCN of the output type class.
	 * @param \ReflectionClass $ref  Reflection of the output type class.
	 *
	...
	private function generate_output_type( string $fqcn, \ReflectionClass $ref ): void {
		...
		foreach ( $ref->getTraits() as $trait ) {
			...
			if ( null !== $trait_info && 'interface' === $trait_info['kind'] ) {
	...
	/**
	 * Generate the GraphQL input object type class for an input type.
	 *
	 * @param string           $fqcn FQCN of the input type class.
	 * @param \ReflectionClass $ref  Reflection of the input type class.
	 *
	...
	/**
	 * Generate the resolver class for a query or mutation command.
	 *
	 * @param string           $fqcn FQCN of the command class.
	 * @param \ReflectionClass $ref  Reflection of the command class.
	 * @param string           $kind Either 'query' or 'mutation'.
	 *
	 * @phpstan-param \ReflectionClass<object> $ref
	 */
	private function generate_resolver( string $fqcn, \ReflectionClass $ref, string $kind ): void {
		...
		foreach ( $params as $param ) {
			...
			if ( null !== $input_info && 'input_type' === $input_info['kind'] ) {
			...
			} elseif ( null !== $input_info && 'enum' === $input_info['kind'] ) {
		...
		$dir_name  = 'query' === $kind ? 'GraphQLQueries' : 'GraphQLMutations';
	...
	private function is_principal_type_compatible( string $declared_type ): bool {
		...
		return ltrim( $this->principal_type, '\\' ) === $declared_type;
	}
	 ...
	 *
	 * @param string            $fqcn             Owning class FQCN, for error messages.
	 * @param \ReflectionMethod $execute_method   The command's `execute()` method.
	 * @param \ReflectionMethod $authorize_method The command's `authorize()` method.
	 *
	 * @return array{
	 ...
	 *
	 * @param \ReflectionParameter $param     The `execute()` parameter.
	 * @param string               $type_name Declared type name of the parameter.
	 */
	 ...
	 *
	 * @param Parameter $param The attribute instance.
	 */
	 ...
	 *
	 * @param string $fqcn      FQCN of the class whose public properties become the args.
	 * @param array  $use_stmts Accumulator of use statements the generated code needs; appended to.
	 *
	 * @return array{fqcn: string, args: list<array<string, mixed>>, properties: list<array{name: string, value_expr: string}>}
	 */
	private function build_unroll_info( string $fqcn, array &$use_stmts ): array {
		...
		foreach ( $ref->getProperties( \ReflectionProperty::IS_PUBLIC ) as $prop ) {
			...
			if ( null !== $class_info && 'enum' === $class_info['kind'] ) {
				// GraphQL engine already resolves enum input values to PHP enum instances,
				// so no ::from() conversion is needed — just assign the value directly.
				...
				$value_expr = "\$args['{$prop_name}']";
				if ( $has_default ) {
					$value_expr .= ' ?? ' . var_export( $default, true ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_var_export -- Design-time code generator emitting a PHP literal, not debug output.
				}
			}
	...
	/**
	 * Describe the static converter method that turns a GraphQL input array into an instance of the given input class, recursing into nested input types.
	 *
	 * @param string $input_fqcn       FQCN of the input type class.
	 * @param array  $input_converters Converters built so far, keyed by input FQCN; nested converters are added to it.
	 *
	 * @return array{method_name: string, input_fqcn: string, input_class: string, properties: list<array{name: string, conversion: ?string}>}
	 */
	private function build_input_converter( string $input_fqcn, array &$input_converters ): array {
		...
		foreach ( $ref->getProperties( \ReflectionProperty::IS_PUBLIC ) as $prop ) {
			...
			if ( null !== $class_info && 'enum' === $class_info['kind'] ) {
				// GraphQL engine already resolves enum input values to PHP enum instances,
				// so no ::from() conversion is needed — just assign the value directly.
			} elseif ( null !== $class_info && 'input_type' === $class_info['kind'] ) {
	...
	/**
	 * Generate the Connection and Edge type classes for a connection node type.
	 *
	 * @param string $node_type_fqcn FQCN of the node type (an output type or an interface trait).
	 */
	...
	/**
	 * Render the PHP source of a Connection type class.
	 *
	 * @param string $php_namespace         Namespace of the generated class.
	 * @param string $node_type_class       Short name of the node's generated type class.
	 * @param string $node_type_namespace   Namespace of the node's generated type class.
	 * @param string $node_type_alias       Alias the node type class is imported under.
	 * @param string $connection_class_name Short name of the Connection class to generate.
	 * @param string $edge_class_name       Short name of the Edge class the connection references.
	 */
	private function generate_connection_code( string $php_namespace, string $node_type_class, string $node_type_namespace, string $node_type_alias, string $connection_class_name, string $edge_class_name ): string {
		$code  = "<?php\n\n";
		$code .= "declare(strict_types=1);\n\n";
		$code .= "// THIS FILE IS AUTO-GENERATED. DO NOT EDIT MANUALLY.\n\n";
		$code .= "namespace {$php_namespace};\n\n";
		...
		$code .= "\t\t\t\t\t'description' => __( 'A connection to a list of {$node_type_class} items.', '{$this->text_domain}' ),\n";
	...
	/**
	 * Render the PHP source of an Edge type class.
	 *
	 * @param string $php_namespace       Namespace of the generated class.
	 * @param string $node_type_class     Short name of the node's generated type class.
	 * @param string $node_type_namespace Namespace of the node's generated type class.
	 * @param string $node_type_alias     Alias the node type class is imported under.
	 * @param string $edge_class_name     Short name of the Edge class to generate.
	 */
	private function generate_edge_code( string $php_namespace, string $node_type_class, string $node_type_namespace, string $node_type_alias, string $edge_class_name ): string {
		$code  = "<?php\n\n";
		$code .= "declare(strict_types=1);\n\n";
		$code .= "// THIS FILE IS AUTO-GENERATED. DO NOT EDIT MANUALLY.\n\n";
		$code .= "namespace {$php_namespace};\n\n";
	...
	/**
	 * Generate the PageInfo type class shared by every connection.
	 */
	...
	/**
	 * Generate the root Query type class listing every query resolver.
	 *
	 * @param array $queries Map of command FQCN => reflection of the query classes.
	 */
	...
	/**
	 * Generate the root Mutation type class listing every mutation resolver.
	 *
	 * @param array $mutations Map of command FQCN => reflection of the mutation classes.
	 */
	...
	/**
	 * Generate the TypeRegistry class mapping interface implementors' PHP classes to their generated GraphQL types.
	 */
	...
	/**
	 * Generate the GraphQLController subclass wired to the detected class, principal and HTTP status resolvers.
	 */
	...
	/**
	 * Write the generation date and the API source hash files into the autogenerated directory.
	 */
	...
	/**
	 * Render a code template with the given variables (plus `text_domain`) extracted into its scope.
	 *
	 * @param string $template_name File name of the template under the code-templates directory.
	 * @param array  $vars          Variables the template reads by name.
	 *
	 * @return string The rendered code.
	 */
	private function render_template( string $template_name, array $vars ): string {
		$vars['text_domain'] = $this->text_domain;
		extract( $vars ); // phpcs:ignore WordPress.PHP.DontExtract.extract_extract -- Templates read their variables by name; the keys are build-time constants set by this class.
		ob_start();
	...
	/**
	 * Read the `#[Description]` text of a class, enum or enum case and apply the metadata description transforms.
	 *
	 * @param \ReflectionClass|\ReflectionEnum|\ReflectionEnumUnitCase $ref Reflector to read the attribute from.
	 *
	 * @phpstan-param \ReflectionClass<object>|\ReflectionEnum<\UnitEnum>|\ReflectionEnumUnitCase $ref
	 */
	...
	/**
	 * Read the `#[Description]` text of a parameter and apply the metadata description transforms.
	 *
	 * @param \ReflectionParameter $param Parameter to read the attribute from.
	 */
	 ...
	 * `__( ..., $text_domain )`, so any default description supplied by a
	 ...
	 *
	 * @phpstan-param \ReflectionClass<object>|\ReflectionEnum<\UnitEnum>|\ReflectionEnumUnitCase|\ReflectionMethod|\ReflectionProperty|\ReflectionParameter $source
	 */
	...
	/**
	 * Look up a discovered class by FQCN.
	 *
	 * @param string $class_name FQCN of the class.
	 *
	 * @return array{class: \ReflectionClass<object>|\ReflectionEnum<\UnitEnum>, kind: string, ignored: bool}|null The class record, or null when the class was not discovered.
	 */
	 ...
	 *
	 * @param \ReflectionProperty $prop                   The property to describe.
	 * @param string              $context                Either 'output' or 'input'.
	 * @param array               $use_stmts              Accumulator of use statements the generated code needs; appended to.
	 * @param array               $type_level_usages      Class-level authorization usages ANDed into the field gate.
	 * @param array               $type_level_descriptors Class-level authorization descriptors merged into the field's descriptors.
	 */
	private function build_field_definition( \ReflectionProperty $prop, string $context, array &$use_stmts, array $type_level_usages = array(), array $type_level_descriptors = array() ): ?array {
		...
		$is_optional = 'input' === $context && ( $nullable || $has_default );
		...
		$is_connection        = Connection::class === $type_name;
		$has_pagination       = $is_connection && ! empty( $args );
		$paginated_connection = 'output' === $context && $has_pagination;
	...
	/**
	 * Translate a property or parameter PHP type into the GraphQL type expression the templates emit, honouring `#[ScalarType]`, `#[ArrayOf]` and `#[ConnectionOf]`.
	 *
	 * @param string                                   $type_name Declared PHP type name, or 'mixed'.
	 * @param bool                                     $nullable  Whether the GraphQL type must be nullable.
	 * @param \ReflectionProperty|\ReflectionParameter $context   The property or parameter carrying the type and its attributes.
	 * @param array                                    $use_stmts Accumulator of use statements the generated code needs; appended to.
	 */
	...
	/**
	 * Translate a type name from an `#[ArrayOf]` attribute into a GraphQL type expression.
	 *
	 * @param string $type      Primitive type name or class FQCN.
	 * @param array  $use_stmts Accumulator of use statements the generated code needs; appended to.
	 */
	...
	/**
	 * Translate a discovered class FQCN into the GraphQL expression of its generated type, falling back to String for unknown classes.
	 *
	 * @param string $fqcn      FQCN of the class.
	 * @param array  $use_stmts Accumulator of use statements the generated code needs; appended to.
	 */
	 ...
	 *
	 * @param array  $class_info Class record as returned by {@see self::get_class_info()}.
	 * @param string $short      Short name of the class.
	 * @param array  $use_stmts  Accumulator of use statements the generated code needs; appended to.
	 */
	...
	/**
	 * Translate a `#[Parameter]` attribute's declared type into a GraphQL type expression.
	 *
	 * @param Parameter $param     The attribute instance.
	 * @param array     $use_stmts Accumulator of use statements the generated code needs; appended to.
	 */
	...
	/**
	 * Translate an `execute()` method's return type into a GraphQL type expression, honouring `#[ReturnType]`, `#[ConnectionOf]` and `#[ArrayOf]`.
	 *
	 * @param \ReflectionMethod $method    The `execute()` method.
	 * @param array             $use_stmts Accumulator of use statements the generated code needs; appended to.
	 */
	private function get_return_type_expr( \ReflectionMethod $method, array &$use_stmts ): string {
		...
		$class_info = $this->get_class_info( $type_name );
		if ( null !== $class_info && 'type' === $class_info['kind'] ) {
			$short       = ( new \ReflectionClass( $type_name ) )->getShortName();
			$alias       = $short . 'Type';
	 ...
	 *
	 * @param string $pascal_case The PascalCase identifier.
	 */
	...
	/**
	 * Run phpcbf with the WordPress-Core standard over a path, or warn when the executable is missing.
	 *
	 * @param string $file_path File or directory to format.
	 */
	private function format_with_phpcbf( string $file_path ): void {
		if ( ! is_executable( $this->phpcbf_path ) ) {
			// A production install of the plugin has no development
			// dependencies, so a consumer building against it has no phpcbf
			// unless it points at its own. The output is functionally
			// identical either way, formatting only affects whitespace.
			echo "Warning: phpcbf not found at {$this->phpcbf_path}; the generated files were left unformatted. Pass --phpcbf-path to format them, or --no-linter to silence this warning.\n";
			return;
		}
