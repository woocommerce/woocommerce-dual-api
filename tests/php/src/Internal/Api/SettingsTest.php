...
class SettingsTest extends WC_Unit_Test_Case {
	...
	/**
	 * @testdox add_settings defines the APQ checkbox with a 'yes' default.
	 */
	...
	/**
	 * @testdox add_settings does not define an endpoint URL field: routes are chosen by each plugin at registration.
	 */
	public function test_add_settings_defines_no_endpoint_url_field(): void {
		$fields = $this->sut->add_settings( array(), Settings::SECTION_ID );
		$by_id  = array_column( $fields, null, 'id' );

		$this->assertArrayNotHasKey( 'woocommerce_graphql_endpoint_url', $by_id );
	}
