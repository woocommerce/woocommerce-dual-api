...
class QueryInfoExtractorTest extends WC_Unit_Test_Case {
	...
	/**
	 * @testdox extract merges a field repeated in the same selection set instead of keeping only the last one.
	 */
	public function test_extract_merges_repeated_fields(): void {
		[ $field ] = $this->parse_top_field( '{ root { product { id } product { name price { amount } } product { price { currency } } } }' );
	...
	/**
	 * @testdox extract merges a field that follows a fragment selecting the same field.
	 */
	public function test_extract_merges_field_following_fragment_with_same_field(): void {
		[ $field, $fragments ] = $this->parse_top_field(
			'{ root { ...D ... { product { sku } } product { id } } } fragment D on Root { product { name } }'
		);

		$tree = QueryInfoExtractor::extract( $field->selectionSet, array(), $fragments );

		$this->assertSame( true, $tree['product']['id'] ?? null );
		$this->assertSame( true, $tree['product']['name'] ?? null );
		$this->assertSame( true, $tree['product']['sku'] ?? null );
	}

	/**
	 * @testdox extract merges repeated inline fragments on the same type.
	 */
	public function test_extract_merges_repeated_typed_inline_fragments(): void {
		[ $field ] = $this->parse_top_field( '{ thing { ... on Widget { color parts { id } } ... on Widget { weight parts { name } } } }' );
