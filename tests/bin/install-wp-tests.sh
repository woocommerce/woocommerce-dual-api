#!/usr/bin/env bash
#
# Install the WordPress test environment, WooCommerce, and WooCommerce's test framework.
#
# Usage: ./install-wp-tests.sh <db-name> <db-user> <db-pass> [db-host] [wp-version] [wc-version] [skip-database-creation]
#
# wc-version may be a version number, 'latest', 'beta'/'rc', or 'nightly' (the current trunk build).
# The installed WooCommerce must be 11.2 or newer (see README.md); the plugin's PHPUnit bootstrap
# finds it at $WP_CORE_DIR/wp-content/plugins/woocommerce unless WC_DIR points elsewhere.
#
...
wait_db() {
	...
	set +e
	mysqladmin status $MYSQLADMIN_FLAGS > /dev/null 2>&1
	while [[ $? -ne 0 ]]; do
		((WAITS++))
		if [ $WAITS -ge 6 ]; then
...
install_woocommerce() {
	# The engine under test must not also ship inside WooCommerce, so the
	# installed WooCommerce must be 11.2 or newer. 'nightly' installs the
	# current trunk build published by WooCommerce, which is what CI uses
	# until 11.2 is released.
	WC_INSTALL_EXTRA=''
	INSTALLED_WC_VERSION=$(wp plugin get woocommerce --field=version 2>/dev/null || echo "")

	if [[ $WC_VERSION == 'nightly' ]]; then
		NIGHTLY_ZIP="$TMPDIR/woocommerce-trunk-nightly.zip"
		download https://github.com/woocommerce/woocommerce/releases/download/nightly/woocommerce-trunk-nightly.zip "$NIGHTLY_ZIP"
		wp plugin install "$NIGHTLY_ZIP" --activate --force
	elif [[ $WC_VERSION == 'beta' ]] || [[ $WC_VERSION == 'rc' ]]; then
		# Get the latest non-trunk version number from the .org repo. This will usually be the latest release, beta, or rc.
		WC_VERSION=$(curl -s https://api.wordpress.org/plugins/info/1.0/woocommerce.json | grep -o '"version":"[^"]*' | sed 's/"version":"//' | head -1)
		wp plugin install woocommerce --activate --force --version=$WC_VERSION
	elif [[ -n $INSTALLED_WC_VERSION ]] && [[ $WC_VERSION == 'latest' ]]; then
		# WooCommerce is already installed, we just must update it to the latest stable version
		wp plugin update woocommerce
		wp plugin activate woocommerce
	else
		if [[ -n $INSTALLED_WC_VERSION ]] && [[ $INSTALLED_WC_VERSION != $WC_VERSION ]]; then
	...
	# Run cron to ensure database tables are installed
...
overlay_wc_test_framework() {
	...
	# framework, so pull just the tests/ tree from the matching source ref with a
	...
	if [[ $WC_VERSION == 'nightly' ]]; then
		...
		wc_ref=$(wp plugin get woocommerce --field=version 2>/dev/null || echo "")
		if [ -z "$wc_ref" ]; then
	...
	echo "Overlaying WooCommerce test framework from source ($wc_ref)..."
	rm -rf "$src_dir"
	git clone --quiet --depth 1 --branch "$wc_ref" --filter=blob:none --sparse \
